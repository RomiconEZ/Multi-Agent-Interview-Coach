...
| Таймаут LLM (превышение `LITELLM_TIMEOUT`) | `LLMClient`: повтор с экспоненциальным backoff до `LITELLM_MAX_RETRIES`. При исчерпании — `LLMClientError` |
...
| Параметр | Описание |
|---|---|
| Стоимость LLM | Два LLM-вызова на ход (Observer + Interviewer) + один вызов на фидбэк (Evaluator). При использовании cloud-моделей (DeepSeek, GPT-4o) — стоимость пропорциональна объёму токенов. Для локальных моделей через LiteLLM — стоимость инфраструктуры |
