...
```python
# src/app/api/v1/__init__.py  (текущее состояние)
from fastapi import APIRouter

router = APIRouter(prefix="/v1")
# endpoint'ы пока не зарегистрированы
```

```python
# src/app/api/__init__.py  (текущее состояние)
from fastapi import APIRouter
...
```python
# src/app/api/__init__.py  (после добавления v2)
from fastapi import APIRouter
from ..api.v1 import router as v1_router
from ..api.v2 import router as v2_router  # будущее
...
> **Примечание**: `main.py` импортирует единый `router` из `api/__init__.py` через `create_application()` в `core/setup.py`. Новые версии подключаются в `api/__init__.py`, а не в `main.py`.
```
...
flowchart TD
    ...
    D --> E["Добавить router_v2<br/>в api/__init__.py"]
...
- [ ] Добавить router в `api/__init__.py` через `router.include_router()`.
