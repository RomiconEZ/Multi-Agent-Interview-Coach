...
class InterviewerAgent(BaseAgent):
    ...
    def _build_response_context(
        ...
        context_parts.extend(
            [
                ...
                "",
                "## ПОСЛЕДНИЙ ВОПРОС/СООБЩЕНИЕ ИНТЕРВЬЮЕРА (АКТИВНЫЙ ЯКОРЬ — ТЕМА ВОПРОСА)",
                last_agent_message,
                "",
                "## СООБЩЕНИЕ КАНДИДАТА",
    ...
    def _get_response_instruction(
        ...
        if analysis.is_gibberish:
            return (
                ...
                "2) ПЕРЕФОРМУЛИРУЙ свой последний технический вопрос (см. 'АКТИВНЫЙ ЯКОРЬ'): "
                "сохрани тему, но измени формулировку, добавь пояснение или подсказку. "
        ...
        if response_type == ResponseType.OFF_TOPIC:
            return (
                ...
                "НЕ поддерживай этот разговор. "
                "1) Скажи: 'Давайте вернёмся к техническим вопросам.' "
                "2) Кратко поясни, что именно ты спрашивал. "
                "3) ПЕРЕФОРМУЛИРУЙ активный вопрос (см. 'АКТИВНЫЙ ЯКОРЬ'): "
                "сохрани тему, но измени формулировку, сделай её яснее или добавь подсказку. "
        ...
        if response_type == ResponseType.QUESTION:
            return (
                ...
                "3) Затем ВЕРНИСЬ К АКТИВНОМУ ТЕХНИЧЕСКОМУ ВОПРОСУ: переформулируй ЕГО ЖЕ "
                "(та же тема, но другими словами или с пояснением) и попроси кандидата ответить. "
        ...
        if response_type == ResponseType.INCOMPLETE:
            ...
            return (
                ...
                "1) Кратко объясни, почему ответ не подходит (что ты спрашивал). "
                "2) ПЕРЕФОРМУЛИРУЙ активный вопрос (см. 'АКТИВНЫЙ ЯКОРЬ'): "
                "сохрани тему, но сделай формулировку яснее или добавь подсказку. "
        ...
        if not analysis.answered_last_question:
            return (
                ...
                "НЕ задавай новый вопрос. "
                "1) Кратко поясни, что ты ожидал ответ на конкретный технический вопрос. "
                "2) ПЕРЕФОРМУЛИРУЙ активный вопрос (см. 'АКТИВНЫЙ ЯКОРЬ'): "
                "сохрани ту же тему, но измени формулировку — сделай её проще, "
                "добавь контекст или подсказку."
            )
    ...
    def _get_hallucination_on_topic_instruction(
        ...
        return (
            ...
            "1) Вежливо укажи на ошибку и кратко объясни, в чём она заключается. "
    ...
    def _get_hallucination_off_topic_instruction(correct_answer: str) -> str:
        ...
        Вопрос не закрыт — после коррекции переформулируем вопрос.
        ...
        return (
            ...
            "1) Вежливо укажи на ошибку и объясни, почему ответ некорректен. "
            f"2) Коротко объясни, как на самом деле (только по теме ошибки): {correct_answer}. "
            "3) НЕ отвечай вместо кандидата на активный технический вопрос. "
            "4) ПЕРЕФОРМУЛИРУЙ активный вопрос (см. 'АКТИВНЫЙ ЯКОРЬ'): "
            "сохрани тему, но измени формулировку, добавь пояснение или подсказку."
        )
