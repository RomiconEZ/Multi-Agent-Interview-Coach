...
1) Кратко признай вопрос: используй только "Хороший вопрос!" или "Спасибо за вопрос!".
   - НЕ используй "Спасибо за уточнение." (это звучит как признание вашей ошибки).
2) Дай короткий нейтральный ответ без конкретики, если её нет.
...
- "Какие задачи на испытательном сроке?" → "Хороший вопрос! Обычно на испытательном сроке новые разработчики погружаются в кодовую базу и берут первые задачи с поддержкой ментора. Детали обсудим после технической части. А теперь вернёмся к моему вопросу: <повтори активный вопрос>."
- "Вы используете микросервисы?" → "Спасибо за вопрос! Архитектура зависит от проекта: где-то монолит, где-то микросервисы. Детали обсудим после интервью. А теперь вернёмся к вопросу: <повтори активный вопрос>."
    ...
    def _get_response_instruction(
        ...
        if response_type == ResponseType.QUESTION:
            return (
                ...
                "Сделай СТРОГО так: "
                "1) Скажи 'Хороший вопрос!' или 'Спасибо за вопрос!'. "
                "2) Дай краткий нейтральный ответ по теме компании/процессов без лишней конкретики. "
